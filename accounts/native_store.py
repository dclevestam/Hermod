...
_NATIVE_ACCOUNTS_FILE = (
    Path(GLib.get_user_config_dir()) / "hermod" / "native-accounts.json"
)
...
def _load_native_accounts_raw():
    ...
    for row in data if isinstance(data, list) else []:
        ...
        merged["provider_kind"] = (
            str(merged.get("provider_kind") or "imap-smtp").strip().lower()
            or "imap-smtp"
        )
        merged["identity"] = str(merged.get("identity") or "").strip()
        merged["presentation_name"] = str(
            merged.get("presentation_name") or merged["identity"] or ""
        ).strip()
...
def upsert_native_account(record):
    ...
    with _native_lock:
        ...
        records.sort(
            key=lambda row: (row.alias or row.presentation_name or row.identity).lower()
        )
...
def remove_native_account(account_id):
    ...
    oauth_provider = (
        str(
            ((removed_record.config if removed_record is not None else {}) or {}).get(
                "oauth_provider"
            )
            or ""
        )
    ...
    revoke_token = str(
        token_bundle.get("refresh_token") or token_bundle.get("access_token") or ""
    ).strip()
    ...
    if removed_record is not None:
        remove_account_preference(
            "native", removed_record.provider_kind, removed_record.identity
        )
    return True
...
class NativeOAuthAccountSource:
    def __init__(self, record: NativeAccountRecord):
        ...
        self._account = _NativeAccountProxy(record)
        self._mail = (
            _NativeMailProxy(record)
            if dict(record.config or {}).get("imap_host")
            else None
        )
...
def get_native_account_descriptors():
    ...
    descriptors.sort(
        key=lambda descriptor: (
            str(getattr(descriptor, "presentation_name", "") or "").lower(),
            descriptor.identity.lower(),
        )
    )
    return descriptors
