...
from accounts.native_store import NativeAccountRecord, NativeOAuthAccountSource, native_descriptor_from_record
...
class NativeStoreTests(unittest.TestCase):
    ...
    def test_native_google_oauth_source_reuses_cached_access_token(self):
        ...
        with mock.patch('accounts.native_store.load_native_oauth_token_bundle', return_value=bundle), \
             mock.patch('accounts.native_store.refresh_google_access_token') as refresh_token:
    ...
    def test_native_google_oauth_source_refreshes_when_cached_token_is_expired(self):
        ...
        with mock.patch('accounts.native_store.load_native_oauth_token_bundle', return_value=stored), \
             mock.patch('accounts.native_store.refresh_google_access_token', return_value=refreshed) as refresh_token, \
             mock.patch('accounts.native_store.store_native_oauth_token_bundle') as store_bundle:
            token = source.get_access_token(network_ready_fn=lambda: True)

        self.assertEqual(token, 'fresh-token')
        refresh_token.assert_called_once_with('client-id.apps.googleusercontent.com', 'refresh-token')
